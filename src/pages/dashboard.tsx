...
import { getHiringRequests } from 'apiSdk/hiring-requests';
import { HiringRequestInterface } from 'interfaces/hiring-request';

import { HiringRequestListPage as ShowCaseList } from 'pages/hiring-requests';

function DashboardPage() {
  const { session } = useSession();

  const engineerStat = useSWR<PaginatedInterface<EngineerInterface>>(
    () => `/engineers`,
    () => getEngineers({ limit: 1, offset: 0 }),
  );

  const hiringRequestStat = useSWR<PaginatedInterface<HiringRequestInterface>>(
    () => `/hiring-requests`,
    () => getHiringRequests({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Engineers',
      result: engineerStat?.data?.totalCount,
      isLoading: engineerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Hiring Requests',
      result: hiringRequestStat?.data?.totalCount,
      isLoading: hiringRequestStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`hiring-requests`} passHref legacyBehavior>
                  <Button
