...
import { getEngineers, deleteEngineerById } from 'apiSdk/engineers';
import { EngineerInterface } from 'interfaces/engineer';

type ColumnType = ColumnDef<EngineerInterface, unknown>;
...
export function EngineerListPage(props: EngineerListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getEngineers({
        relations: ['user', 'organization', 'hiring_request.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['skills.contains', 'experience.contains', 'availability.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<EngineerInterface>>(
    () => `/engineers?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: EngineerInterface) => {
    if (hasAccess('engineer', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/engineers/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('organization', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('hiring_request', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'hiring_request',
          header: 'Hiring Request',
          accessorKey: 'hiring_request',
          cell: ({ row: { original: record } }: any) => record?._count?.hiring_request,
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/engineers/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
              ...
              {hasAccess('engineer', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/engineers/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('engineer', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="engineer" />
        </Flex>

        {hasAccess('engineer', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/engineers/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
